...
$url = "ftp://ad3t0.ddns.net/VPN/$($DOMAIN).ovpn"
$output = "C:\Program Files\OpenVPN\config\$($DOMAIN).ovpn"
Invoke-WebRequest -Uri $url -OutFile $output
...
New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" -Name "OpenVPN-GUI" -Value "C:\Program Files\OpenVPN\bin\openvpn-gui.exe"
